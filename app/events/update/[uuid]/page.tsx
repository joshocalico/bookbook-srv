...
import { 
    getEvent,
    updateEvent as updateEventObject
} from "@/lib/queries";
...
const EventCreatePage = () => {
    ...
    const onSubmit = () => {
        ...
        if (files.length > 0) {
            const body = files[0]
            fetch(`/api/eventimg?filename=event-img-${uuidv4()}`, { body, method: "POST" })
            .then(res => res.json() as unknown as PutBlobResult)
            .then(res => updateEventObject({
                uuid: uuid as string,
                ...event,
                image: res.url,
            }))
    ...
    return <div className="flex flex-col p-4 text-2xl line-clamp-2">
        ...
        { imageUrl && <>
            <label htmlFor="imgcur" className={formLabelStyles}>Current Image</label>
            <Image id="imgcur" className="object-cover rounded-xl w-36 aspect-square mb-6" src={imageUrl} width={200} height={200} alt={"Event image"} />
        </>}

        <div className="mb-4">
            <label htmlFor="title" className={formLabelStyles}>Title</label>
            <input type="text" id="title" disabled={eventLoading} className={formInputStyles} onChange={(e) => updateEvent({ title: e.target.value })} value={title} required></input>
        </div>
        

        <div className="mb-4">
            <label htmlFor="short-desc" className={formLabelStyles}>Short Description</label>
            <input type="text" disabled={eventLoading} className={formInputStyles} id="short-desc" onChange={(e) => updateEvent({ shortDescription: e.target.value })} value={shortDescription} required></input>
        </div>

        <div className="mb-4">
            <label htmlFor="long-desc" className={formLabelStyles}>Long Description</label>
            <input type="text" disabled={eventLoading} className={formInputStyles} id="long-desc" onChange={(e) => updateEvent({ longDescription: e.target.value })} value={longDescription} required></input>
        </div>

        <div className="mb-4 flex">
            <div>
                <label htmlFor="img" className={formLabelStyles}>Image</label>
                <input ref={inputFileRef} disabled={eventLoading} type="file" id="img" accept="image/png, image/jpeg"></input>
            </div>
            <div>
            <label htmlFor="imgclr" className={formLabelStyles}>Image</label>
            <button className={formButtonStyles} disabled={eventLoading} onClick={() => {
        ...
        <button className={formButtonStyles} disabled={eventLoading} onClick={onSubmit}>
            Submit
        </button>

        {
            loading || eventLoading && <p>
                Loading... Please wait.
            </p>
        }
    </div>
